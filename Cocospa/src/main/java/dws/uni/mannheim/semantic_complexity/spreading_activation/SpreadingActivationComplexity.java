...
public class SpreadingActivationComplexity {
	...
	public void runSpreadingActivation(Node seed, Map<Node, Double> activation, Set<Node> idle) {
		...
		for (Entry<Node, Double> e: activation.entrySet()){
			if (e.getValue() > FIRING_THRESHOLD && !burned.contains(e.getKey()) && !idle.contains(e.getKey())) {
				fired.add(e.getKey());
			}
		}
		...
		while(fired.size() > 0){
			...
			System.out.println("Fired.size> 0, so Im spreading the activations....");
			System.out.println("Fired " + fired.size() +":");
			for (Node n : fired){
				System.out.println(n.getProperty("uri")+": "+activation.get(n));
			}
	...
	private void updateActivation(Map<Node, Double> activation, Set<Node> fired, Set<Node> burned, Set<Node> idle, boolean first) {
				...
				if(exclusivity>0.0){
					...
					if (activation.containsKey(neighbour)) {
						if (!burned.contains(neighbour)){
							if (first) 	newActivation = activation.get(neighbour) + exclusivity * importance * activation.get(f);
							else   		newActivation = activation.get(neighbour) + exclusivity * importance * activation.get(f) * ACTIVATION_GRAPH_DECAY ;
						}
						if (idle.contains(neighbour))
							idle.remove(neighbour);
					}
					else if (first) newActivation = exclusivity * importance * activation.get(f);
					else newActivation = exclusivity * importance * activation.get(f)*ACTIVATION_GRAPH_DECAY;

					if (newActivation > 1.0) newActivation = 1.0;
	...
	private Map<Mention, Double> computeActivationAtEncounter(List<Mention> mentions) {
		...
		for (int i =0; i<mentions.size(); i++) {
			...
			if (i>0) {
				int deltaToken = mentions.get(i).getTokenOffsetStart() - mentions.get(i-1).getTokenOffsetEnd();
				System.out.println("applying reading decay...");
				for (Node n: activation.keySet()){
					activation.put(n, activation.get(n)* Math.pow(ACTIVATION_READING_DECAY, deltaToken));
				}
				...
				else result.put(m, 0.0);
			}
			if (i < mentions.size() -1) {
				System.out.println("Spreading from mention: " + mentions.get(i));
	...
	private List<Mention> extractMentions (KanopyDocument ldoc) throws Exception{
		...
		for (Map<String, String> topic : ldoc.topicWordConcept.values()){
			for (Entry<String, String> topicWord : topic.entrySet()){
				...
				forms.add(topicWord.getValue().replace("http://dbpedia.org/resource/", "").replace("_", " ").toLowerCase());
				for (String form: forms) {
					...
					while (m.find()){
						...
						int countPretextTokens = pretext.split("\\W+").length; //splits by any non-alphanumeric. 
						int countMatchedTokens = matchedText.split("\\W+").length;
						mention.setTokenOffsetStart(countPretextTokens +1);
						mention.setTokenOffsetEnd(countPretextTokens + countMatchedTokens);
						...
						Node n = db.findNode(DynamicLabel.label("Resource"), "uri", mention.getMentionedConcept());
						if (n!=null)
							result.add(mention);
					}	
				}
		...
		for (Entry<String, String> isole : ldoc.getIsolatedConcepts().entrySet()){
			String word = isole.getKey();
			Set<String> forms = new HashSet<>();
			forms.add(word);
			forms.addAll(this.formPluralVariations(word));
			forms.add(isole.getValue().replace("http://dbpedia.org/resource/", "").replace("_", " ").toLowerCase());
			for(String form: forms) {
				Pattern p = Pattern.compile("\\b"+form+"\\b");
				Matcher m = p.matcher(text);
				while (m.find()){
	...
	public static void main(String[] args) {
		...
		SpreadingActivationComplexity sac = new SpreadingActivationComplexity(db);

		//List<Mention> mentions = new ArrayList<>();
		KanopyDocumentJsonReader reader = new KanopyDocumentJsonReader();
		try{
			KanopyDocument doc= reader.readDocument("17century-selfies.en.1.txt.kanopy.json");
			
			
			doc.docPath = "17century-selfies.en.1.txt.kanopy.json";
			...
			Comparator<Mention> cmp = new Comparator<Mention>() {
				...
				public int compare(Mention o1, Mention o2) {
					if (o1.getCharOffsetStart() < o2.getCharOffsetStart())
						return -1;
					else if (o1.getCharOffsetStart() == o2.getCharOffsetStart()) {
						if (o1.getCharOffsetEnd() < o2.getCharOffsetEnd())
							return -1;
						else if (o1.getCharOffsetEnd()>o2.getCharOffsetEnd())
							return 1;
						else return 0;
					}
					else 
		...
		m1.setMentionedConcept("http://dbpedia.org/resource/Fatigue_(medical)");
		m1.setTokenOffsetStart(11);
		m1.setTokenOffsetEnd(12);



		Mention m2 = new Mention();
		m2.setMentionedConcept("http://dbpedia.org/resource/Attention_deficit_hyperactivity_disorder");
		m2.setTokenOffsetStart(31);
		m2.setTokenOffsetEnd(32);


		Mention m3 = new Mention();
		m3.setMentionedConcept("http://dbpedia.org/resource/Attention");
		m3.setTokenOffsetStart(33);
		m3.setTokenOffsetEnd(34);

		Mention m7 = new Mention();
		m7.setMentionedConcept("http://dbpedia.org/resource/Attention");
		m7.setTokenOffsetStart(43);
		m7.setTokenOffsetEnd(44);


		Mention m4 = new Mention();
		m4.setMentionedConcept("http://dbpedia.org/resource/School");
		m4.setTokenOffsetStart(53);
		m4.setTokenOffsetEnd(54);

		Mention m8 = new Mention();
		m8.setMentionedConcept("http://dbpedia.org/resource/United_States");
		m8.setTokenOffsetStart(57);
		m8.setTokenOffsetEnd(59);



		Mention m5 = new Mention();
		m5.setMentionedConcept("http://dbpedia.org/resource/Teacher");
		m5.setTokenOffsetStart(63);
		m5.setTokenOffsetEnd(64);


		Mention m6 = new Mention();
		m6.setMentionedConcept("http://dbpedia.org/resource/Education");
		m6.setTokenOffsetStart(73);
		m6.setTokenOffsetEnd(74);
	...
	//	mentions.add(m4);
		...
		mentions.add(m6);
			 */		
				...
				Map<Mention, Double> res = sac.computeActivationAtEncounter(mentions);
				for(Entry<Mention, Double> re : res.entrySet()) {
					System.out.println(re.getKey().getMentionedConcept() + "->" + re.getValue());
				}
			}catch(Exception ex){
				ex.printStackTrace();
			}
