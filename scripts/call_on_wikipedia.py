...
with open('wikipedia_eval.csv', 'w') as fout:
    ...
    for (article_name, simple_text), (_, complecs_text) in zip(articles_simple_wikipedia.items(), articles_complecs_wikipedia.items()):
        simp_score = get_complexity_from_api(simple_text)
        comp_score = get_complexity_from_api(complecs_text)
        print (article_name, '\t simple: ', simp_score, '\t complex: ', comp_score)
        fout.write('\t'.join([article_name, str(simp_score), str(comp_score)]) + '\n')
        break

#with open(complecs_wiki, 'r', encoding='utf-8') as fin:
#    complecs = fin.readline()
